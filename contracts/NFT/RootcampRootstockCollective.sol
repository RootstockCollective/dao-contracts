...
/**
 * @title Rootcamp Certified Builder NFT Collection
 * @notice Non-transferrable POAP-style NFTs for Rootcamp program graduates
 * @dev Whitelisted users can mint once per address. Requires minimum stRIF balance.
 */
contract RootcampRootstockCollective is
  Initializable,
  ERC721Upgradeable,
  ...
  UUPSUpgradeable
{
  ...
  event RootcampNftUnderlyingTokenChanged(IERC20 indexed oldToken, IERC20 indexed newToken);
  event RootcampNftTokenThresholdChanged(uint256 indexed oldThreshold, uint256 indexed newThreshold);
  event RootcampNftMaxSupplyChanged(uint256 indexed oldMaxSupply, uint256 indexed newMaxSupply);
  event RootcampNftFolderIpfsCidChanged(string oldCid, string newCid);

  // ERRORS

  error RootcampNftOutOfTokens(uint256 maxSupply);
  error RootcampNftAdminRoleViolation();
  error RootcampNftTransfersDisabled();
  error RootcampNftBelowTokenThreshold(uint256 balance, uint256 threshold);
  ...
  function initialize(
    ...
    __ERC721_init("Rootcamp Certified Builder", "RRC");
  ...
  function mint() external onlyRole(MINTER_ROLE) returns (uint256) {
    ...
    if (tokensAvailable() == 0) revert RootcampNftOutOfTokens(maxSupply);
    if (balance < underlyingTokenThreshold)
      revert RootcampNftBelowTokenThreshold(balance, underlyingTokenThreshold);
    // minting
  ...
  function setFolderIpfsCid(string calldata cid) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit RootcampNftFolderIpfsCidChanged(oldCid, cid);
  }
  ...
  function setMaxSupply(uint256 newMaxSupply) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    if (newMaxSupply < maxSupply) revert RootcampNftInvalidMaxSupply(newMaxSupply, maxSupply);
    uint256 oldMaxSupply = maxSupply;
    maxSupply = newMaxSupply;
    emit RootcampNftMaxSupplyChanged(oldMaxSupply, newMaxSupply);
  }
  ...
  function setTokenThreshold(uint256 threshold) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    emit RootcampNftTokenThresholdChanged(oldThreshold, threshold);
  }
  ...
  function setUnderlyingToken(IERC20 newToken) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    if (newToken == IERC20(address(0))) revert RootcampNftInvalidAddress(address(newToken));
    IERC20 oldToken = underlyingToken;
    underlyingToken = newToken;
    emit RootcampNftUnderlyingTokenChanged(oldToken, newToken);
  }
   ...
   * @dev Intended for frontend/dApp usage only due to potential gas costs with large arrays.
  ...
  /// @dev Returns a single metadata file for all minted tokens
  ...
  function _update(
    ...
    if (from != address(0) && to != address(0)) revert RootcampNftTransfersDisabled();

    return super._update(to, tokenId, auth);
  }

  /// @dev Prevents granting roles to zero address.
  function _grantRole(bytes32 role, address receiver) internal virtual override returns (bool) {
    if (receiver == address(0)) revert RootcampNftAdminRoleViolation();
